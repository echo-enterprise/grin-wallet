...
use crate::echo_core::core::transaction::{FeeFields, OutputFeatures};
use crate::echo_core::ser as echo_ser;
use crate::echo_core::ser::{Readable, Reader, Writeable, Writer};
use crate::echo_keychain::BlindingFactor;
use crate::echo_util::secp::key::PublicKey;
use crate::echo_util::secp::pedersen::{Commitment, RangeProof};
...
impl Writeable for SlateStateV4 {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), echo_ser::Error> {
		let b = match self {
...
impl Readable for SlateStateV4 {
	fn read<R: Reader>(reader: &mut R) -> Result<SlateStateV4, echo_ser::Error> {
...
impl Readable for UuidWrap {
	fn read<R: Reader>(reader: &mut R) -> Result<UuidWrap, echo_ser::Error> {
...
impl Readable for SlateOptFields {
	fn read<R: Reader>(reader: &mut R) -> Result<SlateOptFields, echo_ser::Error> {
...
impl Readable for SigsWrap {
	fn read<R: Reader>(reader: &mut R) -> Result<SigsWrap, echo_ser::Error> {
...
impl Readable for SlateOptStructs {
	fn read<R: Reader>(reader: &mut R) -> Result<SlateOptStructs, echo_ser::Error> {
		let status = reader.read_u8()?;
...
impl Readable for ComsWrap {
	fn read<R: Reader>(reader: &mut R) -> Result<ComsWrap, echo_ser::Error> {
...
impl Readable for ProofWrap {
	fn read<R: Reader>(reader: &mut R) -> Result<ProofWrap, echo_ser::Error> {
...
impl serde::Serialize for SlateV4Bin {
	...
	{
		let mut vec = vec![];
		echo_ser::serialize(&mut vec, echo_ser::ProtocolVersion(4), self)
		...
		D: serde::Deserializer<'de>,
	{
		...
		impl<'de> serde::de::Visitor<'de> for SlateV4BinVisitor {
				...
				E: serde::de::Error,
			{
				...
				let s = echo_ser::deserialize(
					&mut reader,
					echo_ser::ProtocolVersion(4),
					echo_ser::DeserializationMode::default(),
				)
...
impl Readable for SlateV4Bin {
	fn read<R: Reader>(reader: &mut R) -> Result<SlateV4Bin, echo_ser::Error> {
...
fn slate_v4_serialize_deserialize() {
	use crate::echo_util::from_hex;
	use crate::echo_util::secp::key::PublicKey;
	use crate::Slate;
	use echo_core::global::{set_local_chain_type, ChainTypes};
	use echo_keychain::{ExtKeychain, Keychain, SwitchCommitmentType};
	...
	let mut vec = Vec::new();
	let _ = echo_ser::serialize_default(&mut vec, &v4_bin).expect("serialization failed");
	let b4_bin_2: SlateV4Bin = echo_ser::deserialize_default(&mut &vec[..]).unwrap();
